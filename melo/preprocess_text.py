...
def main(
            ...
            with open(metadata, 'r', encoding='utf-8') as in_file:
                for line in tqdm(in_file.readlines()):
                    try:
                        ...
                        # Create a new path in the writable /kaggle/working/ directory
                        output_bert_path = os.path.join('/kaggle/working/bert_features/', os.path.relpath(bert_path, '/kaggle/input/'))
                        # Ensure the new directory exists
                        os.makedirs(os.path.dirname(output_bert_path), exist_ok=True)
                        # Save the file to the new, correct path
                        
                        # torch.save(bert.cpu(), output_bert_path)
    ...
    # The rest of the script for creating train/val splits and config.
