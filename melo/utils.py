...
def get_text_for_tts_infer(text, language_str, hps, device, symbol_to_id=None):
    ...
    if getattr(hps.data, "disable_bert", False):
        ...
        # Inference-only patch for Romanian
        if language_str == "RO":
            if bert.shape[-1] > len(phone):
                bert = bert[:, :len(phone)]
            elif bert.shape[-1] < len(phone):
                pad = torch.zeros(bert.shape[0], len(phone) - bert.shape[1], device=bert.device)
                bert = torch.cat([bert, pad], dim=1)
