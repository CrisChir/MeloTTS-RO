...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    def __init__(self, audiopaths_sid_text, hparams):
        ...
        from melo.text.symbols import language_id_map
    ...
    def _filter(self):
        ...
        for line in self.audiopaths_sid_text:
            ...
            audiopath, sid, language, text, phones, tones, word2ph = line
            if self.min_text_len <= len(phones.split(" ")) <= self.max_text_len:
                audiopaths_sid_text_new.append(line)
                lengths.append(os.path.getsize(audiopath) // (2 * self.hop_length))
        self.audiopaths_sid_text = audiopaths_sid_text_new
        self.lengths = lengths

    def get_audio(self, filename):
        audio, sampling_rate = utils.load_wav_to_torch(filename, self.sampling_rate)
    ...
    def get_text(self, text, phones, tones, language_str, wav_path):
        phone_list = phones.split(" ")
        tone_list = [int(i) for i in tones.split(" ")]
        phone_ids, tone_ids, language_ids = cleaned_text_to_sequence(phone_list, tone_list, language_str)
        ...
        if self.use_bert:
            ...
            try:
                ...
                print(f"FATAL ERROR: Could not load BERT file {bert_path}", e)
                raise
        return bert, ja_bert, torch.LongTensor(phone_ids), torch.LongTensor(tone_ids), torch.LongTensor(language_ids)

    def get_audio_text_speaker_pair(self, line):
        wav_path, sid, language_str, text, phones, tones, word2ph = line
        bert, ja_bert, phone_ids, tone_ids, language_id = self.get_text(text, phones, tones, language_str, wav_path)
        spec, wav = self.get_audio(wav_path)
        speaker_id = self.spk2id[sid]
        speaker_id = torch.LongTensor([int(speaker_id)])
        return (phone_ids, tone_ids, language_id, spec, wav, speaker_id, bert, ja_bert)
...
class TextAudioSpeakerCollate:
    ...
    def __call__(self, batch):
        _, ids_sorted_decreasing = torch.sort(torch.LongTensor([x[3].size(1) for x in batch]), dim=0, descending=True)
        ...
        if batch[0][6] is not None:
            max_bert_len = max([x[6].shape[0] for x in batch])
            bert_padded = torch.FloatTensor(len(batch), 1024, max_text_len)
            bert_padded.zero_()

        ja_bert_padded = None
        if batch[0][7] is not None:
            max_ja_bert_len = max([x[7].shape[0] for x in batch])
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            text, tone, lang, spec, wav, sid_val, bert, ja_bert = row
            
            text_padded[i, :text.size(0)] = text
            text_lengths[i] = text.size(0)
            
            tone_padded[i, :tone.size(0)] = tone
            language_padded[i, :lang.size(0)] = lang
            ...
            if bert is not None:
                bert_padded[i, :, :text.size(0)] = bert.t()
            if ja_bert is not None:
                ja_bert_padded[i, :, :text.size(0)] = ja_bert.t()

        if self.return_ids:
            return text_padded, text_lengths, tone_padded, language_padded, spec_padded, spec_lengths, wav_padded, wav_lengths, sid, bert_padded, ja_bert_padded, ids_sorted_decreasing
        return text_padded, text_lengths, tone_padded, language_padded, spec_padded, spec_lengths, wav_padded, wav_lengths, sid, bert_padded, ja_bert_padded
# import os
# import random
# import torch
# import torch.utils.data
