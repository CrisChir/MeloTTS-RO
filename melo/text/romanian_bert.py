...
def get_bert_feature(text, word2ph, device=None, phone_len=None):
    ...
    - phone_len: Optional. If provided, aligns output to this length (used only in inference).
    """
    ...
    bert_features = []
    num_bert_tokens = last_hidden_state.shape[0] - 2  # exclude [CLS] and [SEP]
    word2ph = word2ph[:num_bert_tokens]  # safeguard
    ...
    # Optional alignment for inference only
    if phone_len is not None:
        if bert_tensor.shape[-1] > phone_len:
            ...
            pad = torch.zeros(bert_tensor.shape[0], phone_len - bert_tensor.shape[1], device=bert_tensor.device)
